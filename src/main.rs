...
fn main() {
    ...
    if first == "build" || first == "compile" {
        ...
        match std::fs::read_to_string(&source_path).map_err(|error| error.to_string()).and_then(|source| basisread::compile_source(&source, &output_path, include_str!("lib.rs"), include_str!("lexer.rs")).map_err(|error| error.to_string())) {
